...
AI Recommendation is an optional second recommendation path. Supabase remains
the memory layer: AI Barista summarizes the loaded user's profile, top liked and
disliked ingredients, last five ratings, top five favorites, recent sessions,
and relevant custom drinks. OpenAI then reasons over that concise memory plus
today's context and the current catalog.

OpenAI does not replace the standard rule-based recommendation engine. If
`OPENAI_API_KEY` is missing or generation fails, the app shows a friendly
message and falls back to standard recommendations. AI responses use structured
JSON, prefer catalog drinks, avoid disliked ingredients, and never claim
...
Optionally set `OPENAI_MODEL` to override the default model. Never commit keys
or a populated `.streamlit/secrets.toml` file to source control.
